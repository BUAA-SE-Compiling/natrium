...
每个代码块（`block_stmt`）都是一级作用域。每级作用域内部的变量声明不能重复。作用域内声明的变量可以覆盖上一级作用域中的变量。

## 变量声明增强

加分：5pt

在每一级作用域中，你不仅可以在作用域顶端声明变量，也能在作用域中间声明变量。变量在声明之前无法使用。
...
`break` 和 `continue` 必须在循环体内使用，在其他地方使用是编译错误。`break` 代表跳出循环体，控制转移到循环外的下一条语句。`continue` 代表跳过本次循环体的代码，控制转移到循环体的最后一条语句。
